...
Object.defineProperty(exports, "__esModule", {
    value: !0
}), exports.createSparkLineCellGroup = void 0;

const vrender_1 = require("./../../../vrender"), vscale_1 = require("@visactor/vscale"), vutils_1 = require("@visactor/vutils"), group_1 = require("../../graphic/group"), cell_border_stroke_width_1 = require("../../utils/cell-border-stroke-width"), xScale = new vscale_1.PointScale, yScale = new vscale_1.LinearScale;
...
function createSparkLine(col, row, width, height, padding, table) {
    ...
    if (yScale.range([ 0, height ]), "object" == typeof sparklineSpec.xField && Array.isArray(sparklineSpec.xField.domain)) {
        ...
        for (let i = 0; i < domain.length; i++) {
            ...
            for (let j = 0; j < values.length; j++) if (domain[i] === values[j]) {
                ...
                if (!(0, vutils_1.isValid)(data[xField]) || !(0, vutils_1.isValid)(data[yField])) break;
                items.push({
                    x: 0 + xScale.scale(data[xField]),
                    y: bottom - yScale.scale(data[yField]),
                    defined: (0, vutils_1.isValid)(data[yField])
        ...
        items.push({
            ...
            defined: (0, vutils_1.isValid)(yField ? data[yField] : data),
            rawData: data
        }), dataItems.push(data);
    }
    ...
    if (symbolGroup) {
        ...
        if ("all" === sparklineSpec.pointShowRule) for (let i = 0; i < items.length; i++) {
            ...
            if (defined) {
                const symbol = (0, vrender_1.createSymbol)({
                    x: x,
                    y: y
                });
...
function createChartGroup(spec, x, y, width, height) {
    ...
    if (group.name = "sparkline", "line" === specObj.type) {
        ...
        const symbolGroup = new group_1.Group({
            x: 0,
            y: 0,
            width: width,
            height: height,
...
function getYNumbers(data, field) {
    ...
    for (let i = 0; i < data.length; i++) {
        const item = data[i];
        (0, vutils_1.isValid)(field) && (0, vutils_1.isValid)(item[field]) ? numbers.push(item[field]) : !(0, 
        vutils_1.isValid)(field) && (0, vutils_1.isValid)(item) && numbers.push(item);
    }
    return numbers;
}

exports.createSparkLineCellGroup = createSparkLineCellGroup;
