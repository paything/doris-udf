...
import { changeCheckboxOrder, initCheckedState, initLeftRecordsCheckState, setCheckedState, setHeaderCheckedState, syncCheckedState, updateHeaderCheckedState } from "./checkbox/checkbox";
...
export class StateManager {
    ...
    setSelectState() {
        ...
        this.select.highlightScope = !0 === disableSelect ? HighlightScope.none : "cross" === highlightMode ? HighlightScope.cross : "row" === highlightMode ? HighlightScope.row : "column" === highlightMode ? HighlightScope.column : HighlightScope.single, 
    ...
    isSelected(col, row) {
        ...
        return this.select.ranges.forEach((range => {
            cellInRange(range, col, row) && (seled = !0);
        })), seled;
    }
    ...
    updateSelectPos(col, row, isShift = !1, isCtrl = !1, isSelectAll = !1, makeSelectCellVisible = !0, skipBodyMerge = !1) {
        ...
        row > this.table.rowCount - 1 && (row = this.table.rowCount - 1), updateSelectPosition(this, col, row, isShift, isCtrl, isSelectAll, makeSelectCellVisible, skipBodyMerge);
    }
    ...
    endSelectCells(fireListener = !0, fireClear = !0) {
        if (this.select.selecting) {
            ...
            isSame ? (this.select.ranges.pop(), deleteAllSelectingBorder(this.table.scenegraph), 
            this.table.scenegraph.selectingRangeComponents.clear()) : selectEnd(this.table.scenegraph);
    ...
    updateVerticalScrollBar(yRatio) {
        ...
        this.updateHoverPos(-1, -1), this.table.fireListeners(TABLE_EVENT_TYPE.SCROLL, {
            event: void 0,
            scrollTop: this.scroll.verticalBarPos,
            scrollLeft: this.scroll.horizontalBarPos,
    ...
    showContextMenu(col, row, x, y) {
        ...
        this.menu.x = x, this.menu.y = y, this.table.scenegraph.component.menu.attach(x, y, col, row, MenuType.contextmenu)), 
        this.table.fireListeners(TABLE_EVENT_TYPE.SHOW_MENU, {
            x: x,
            y: y,
            col: col,
            row: row,
    ...
    showDropDownMenu(col, row, x, y) {
        ...
        this.menu.bounds = this.table.scenegraph.component.menu.bounds), this.table.fireListeners(TABLE_EVENT_TYPE.SHOW_MENU, {
            x: x,
            y: y,
            col: col,
            row: row,
        ...
        let icon;
        cellGroup.forEachChildren((child => child.attribute.funcType === IconFuncTypeEnum.dropDown && (icon = child, 
    ...
    hideMenu() {
        this.menu.isShow && (this.table.fireListeners(TABLE_EVENT_TYPE.DROPDOWN_MENU_CLEAR, null), 
        this.table.fireListeners(TABLE_EVENT_TYPE.HIDE_MENU, null), this.menu.isShow = !1, 
        ...
        if (Array.isArray(highlights)) for (let i = 0; i < highlights.length; i++) {
            ...
            if (isValid(col) && isValid(row) && this.table.isCellRangeEqual(colNow, rowNow, col, row)) {
                ...
                if ("function" == typeof dropDownMenu && (dropDownMenu = dropDownMenu({
                    ...
                    if (isObject(item) && (item.menuKey || item.text) === (menuKey || "") && i === index) return !0;
                    if (isString(item) && item === menuKey && i === index) return !0;
                }
            }
        }
        return !1;
    ...
    updateSortState(sortState) {
        ...
        for (let index = 0; index < sortState.length; index++) {
            ...
            let iconMark;
            traverseObject(cellGroup, "children", (mark => "sort" === mark.attribute.funcType && (iconMark = mark, 
    ...
    updateDrillState(dimensionKey, title, drillDown, drillUp, col, row) {
        ...
        this.drill.drillUp = drillUp, this.drill.col = col, this.drill.row = row, updateDrill(col, row, drillDown, drillUp, this.table);
    }
    updateSparklineHoverPose(col, row, x, y) {
        ...
        let isUpdated = !1;
        -1 !== col && -1 !== row && (isUpdated = updateChartHover(col, row, x, y, this.table)), 
