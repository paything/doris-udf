...
export function startMoveCol(col, row, x, y, state, event) {
    var _a;
...
export function updateMoveCol(col, row, x, y, state, event) {
    if (!("canMoveHeaderPosition" in state.table.internalProps.layoutMap)) return;
    ...
    if (state.table.internalProps.layoutMap.canMoveHeaderPosition({
        ...
        state.table.scenegraph.component.updateMoveCol(backX, lineX, backY, lineY), state.table.fireListeners(TABLE_EVENT_TYPE.CHANGING_HEADER_POSITION, {
            col: col,
            row: row,
            x: x,
            y: y,
...
export function endMoveCol(state) {
    var _a, _b;
    let moveColResult = !1;
